(function(){
  ...
  function freshProgress() {
    return {
      ...
      env7Answered: false,
      env8Answered: false,
      env9Judged: [false, false, false, false, false],
      env9Complete: false,
      env11Seen: {},
      reflection: ''
    };
  ...
  function resetAll() {
    ...
    // Environment 9, the five average judgements
    for (var tn = 1; tn <= 5; tn++) {
      var tcard = document.getElementById('trustCard' + tn);
      if (tcard) tcard.hidden = (tn !== 1);
      var tfb = document.getElementById('trustFb' + tn);
      if (tfb) { tfb.classList.remove('show', 'good', 'think'); tfb.innerHTML = ''; }
      var tnext = document.getElementById('trustNext' + tn);
      if (tnext) tnext.classList.remove('show');
    }
    document.querySelectorAll('#screen-8 .trust-opt').forEach(function (b) {
      b.classList.remove('is-selected', 'is-correct', 'is-incorrect');
    });
    document.querySelectorAll('#screen-8 .trust-pip').forEach(function (p, idx) {
      p.classList.remove('is-done');
      p.classList.toggle('is-current', idx === 0);
    });
    var trustRule = document.getElementById('trustRule');
    if (trustRule) trustRule.classList.remove('show');
    var trustCount = document.getElementById('trustCount');
    if (trustCount) trustCount.textContent = 'Situation 1 of 5';

    // Environment 11, formula reveal
    document.querySelectorAll('.formula-step-btn').forEach(function (b) { b.classList.remove('is-active'); });
    ['fpLim', 'fpTop', 'fpBot'].forEach(function (id) {
      var el = document.getElementById(id);
      if (el) el.classList.remove('lit');
    });
    var fex = document.getElementById('formulaExplain');
    if (fex) fex.textContent = 'Choose a part of the expression above to see what it means.';
    var ffin = document.getElementById('formulaFinal');
    if (ffin) ffin.classList.remove('show');

    // Environment 10, reflection
  ...
  /* ============================================================
     ...
     ============================================================ */
  var SHARED_MEDIA = {
    waterVideo: 'env04-05-water-bucket.mp4',
    waterPoster: 'env04-05-water-bucket-poster.jpg',
    phoneClipA: 'scene07-phone-a-20-points-per-min.mp4',
    phonePosterA: 'scene07-phone-a-poster.jpg',
    phoneClipB: 'scene07-phone-b-10-points-per-min.mp4',
    phonePosterB: 'scene07-phone-b-poster.jpg'
  };
  ...
  /* ============================================================
     ENVIRONMENT 9: WHEN CAN WE TRUST AN AVERAGE?

     Replaces the old Group A / Group B sort. That activity asked
     learners to sort by "what feels similar" with no criterion, and
     its two groups were really the differential and integral split,
     which belongs to a later phase rather than to Phase 0.

     Here the learner judges five averages they have already met and
     derives the actual rule: an average can be trusted when the rate
     holds steady and hides things when the rate varies.

     Nothing is moved in the DOM when an answer is given, so keyboard
     focus stays where the learner put it. The old version moved chips
     between trays and destroyed focus on every click.
     ============================================================ */
  (function environment9() {
    var TOTAL_SITUATIONS = 5;
    var cards = [];
    for (var i = 1; i <= TOTAL_SITUATIONS; i++) {
      cards.push(document.getElementById('trustCard' + i));
    }
    if (!cards[0]) return;

    var pips = document.querySelectorAll('#screen-8 .trust-pip');
    var count = document.getElementById('trustCount');
    var rule = document.getElementById('trustRule');

    var FEEDBACK = {
      1: { right: "Right. You heard what really happened: twenty minutes at a standstill near the bridge, then long stretches above 100. The average is correct and describes no moment of that trip.", wrong: "Think back to Amina's journey. She was stopped completely for twenty minutes and well over 100 afterwards. An average of 60 covers up both of those." },
      2: { right: "Yes. You watched that rate swing between 0.5 and 2.5 litres an hour as different appliances came on. The average of 1.5 sits in the middle and hides every one of those swings.", wrong: "You dragged the slider through those five hours yourself. The rate right now went from 0.5 up to 2.5 and back down to 1.0. An average of 1.5 tells you none of that." },
      3: { right: "Correct. Nothing about this flow changes, so every single minute looked like the average. Here the average really does describe each moment.", wrong: "Look at the word steady. The flow never changed, so each minute was identical to the average. This is the case where an average can be trusted completely." },
      4: { right: "Yes. If the drain sped up as it went, then early minutes were slower than 10 and later ones faster. No single moment matched the average.", wrong: "The clue is that the draining changed as the hour went on. Whenever the rate shifts, the average stops describing any particular moment." },
      5: { right: "Correct. With no stops and no traffic, the pace held roughly constant, so the average is a fair description of the whole journey and of most moments in it.", wrong: "There were no stops and no traffic here, so the speed barely changed. When the rate holds steady, the average and the moment agree." }
    };

    function showCard(n) {
      cards.forEach(function (c, idx) {
        if (c) c.hidden = (idx !== n - 1);
      });
      pips.forEach(function (pip, idx) {
        pip.classList.toggle('is-current', idx === n - 1);
      });
      if (count) count.textContent = 'Situation ' + n + ' of ' + TOTAL_SITUATIONS;
    }

    function markPipDone(n) {
      if (pips[n - 1]) pips[n - 1].classList.add('is-done');
    }
    ...
    cards.forEach(function (card, idx) {
      ...
      var n = idx + 1;
      var answer = card.dataset.answer;
      var fb = document.getElementById('trustFb' + n);
      var next = document.getElementById('trustNext' + n);

      card.querySelectorAll('.trust-opt').forEach(function (btn) {
        btn.addEventListener('click', function () {
          var isCorrect = btn.dataset.choice === answer;
          card.querySelectorAll('.trust-opt').forEach(function (x) {
            x.classList.remove('is-selected', 'is-correct', 'is-incorrect');
          });
          btn.classList.add('is-selected', isCorrect ? 'is-correct' : 'is-incorrect');

          // Feedback carries an icon and a word as well as a colour, so it
          // never relies on colour alone to signal right or wrong.
          var icon = isCorrect ? 'ic-check' : 'ic-spark';
          var lead = isCorrect ? 'Correct. ' : 'Not quite. ';
          var text = isCorrect ? FEEDBACK[n].right : FEEDBACK[n].wrong;
          fb.innerHTML = '<svg class="icon" aria-hidden="true"><use href="#' + icon + '"></use></svg><span>' + text + '</span>';
          fb.classList.remove('good', 'think');
          fb.classList.add('show', isCorrect ? 'good' : 'think');
          fb.setAttribute('aria-label', lead + text);

          if (isCorrect) {
            markPipDone(n);
            if (next) next.classList.add('show');
            State.progress.env9Judged[n - 1] = true;
          }
        });
      });
      ...
      if (next) {
        next.addEventListener('click', function () {
          var target = parseInt(next.dataset.next, 10);
          if (target > TOTAL_SITUATIONS) {
            // The judgements are done, so the cards give way to the rule
            // they produced rather than sitting behind it.
            cards.forEach(function (c) { if (c) c.hidden = true; });
            if (rule) rule.classList.add('show');
            if (count) count.textContent = 'All 5 judged';
            State.progress.env9Complete = true;
          } else {
            showCard(target);
          }
        });
  ...
  /* ============================================================
     ENVIRONMENT 11: A PRECISE LANGUAGE FOR THE IDEA

     The notation appears here and nowhere earlier, because only now
     has the learner met the problem it answers. Each part is tied to
     something they actually did in this phase.

     Note on accuracy: h is an amount added to the input, not "time".
     In the journeys of this phase the input happened to be time, but
     the expression itself is about a change in the input and the
     matching change in the output, with the interval shrinking
     towards zero rather than ever being zero.
     ============================================================ */
  (function environment11() {
    var explain = document.getElementById('formulaExplain');
    var finalNote = document.getElementById('formulaFinal');
    if (!explain) return;

    var parts = { lim: document.getElementById('fpLim'), top: document.getElementById('fpTop'), bot: document.getElementById('fpBot') };

    var TEXT = {
      top: 'This is the amount of change, exactly what you measured with the bucket. The reading after, f(x + h), minus the reading before, f(x). Eight litres minus two litres gave you six.',
      bot: 'This is the size of the interval you measured across. It is the six seconds against the eighteen seconds for the two taps, the thing that separated them when the amount alone could not.',
      lim: 'This is the rescue. Rather than dividing by an interval of nothing, we ask what the answer settles towards as the interval h is made smaller and smaller. It never reaches zero, it approaches it.',
      all: 'Read as a whole: the change in the output, divided by the change in the input, as that interval shrinks towards nothing. It is the rate right now, the thing an average could not give you.'
    };
    var LIT = { top: ['top'], bot: ['bot'], lim: ['lim'], all: ['lim','top','bot'] };

    document.querySelectorAll('.formula-step-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var key = btn.dataset.part;
        document.querySelectorAll('.formula-step-btn').forEach(function (b) { b.classList.remove('is-active'); });
        btn.classList.add('is-active');
        Object.keys(parts).forEach(function (k) {
          if (parts[k]) parts[k].classList.toggle('lit', LIT[key].indexOf(k) !== -1);
        });
        explain.textContent = TEXT[key];
        State.progress.env11Seen[key] = true;
        if (key === 'all' && finalNote) finalNote.classList.add('show');
      });
    });
  })();
